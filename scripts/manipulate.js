...
async function main() {
  ...
  const data = {
    "Price Before": `1 ${ARB_FOR.symbol} = ${Number(priceBefore).toFixed(0)} ${
      ARB_AGAINST.symbol
    }`,
    "Price After": `1 ${ARB_FOR.symbol} = ${Number(priceAfter).toFixed(0)} ${
      ARB_AGAINST.symbol
    }`,
  };
...
async function manipulatePrice(_path) {
  ...
  const approval = await _path[0].contract
    .connect(signer)
    .approve(await EXCHANGE_TO_USE.router.getAddress(), amount, {
      gasLimit: 125000,
    });
  await approval.wait();
